...
ensure_agent_sessions() {
    ...
    for agent in "${agents[@]}"; do
        if ! $TMUX_BIN has-session -t "$agent" 2>/dev/null; then
            ...
            $TMUX_BIN new-session -d -s "$agent" -c "$PROJECT_ROOT"
            sleep 1
        else
...
simple_init_agents() {
    ...
    for agent in "${agents[@]}"; do
        ...
        $TMUX_BIN send-keys -t "$agent" 'bash' Enter
        sleep 1
        
        # Imposta il working directory
        $TMUX_BIN send-keys -t "$agent" "cd $PROJECT_ROOT" Enter
        sleep 0.5
        
        # Banner di inizializzazione
        $TMUX_BIN send-keys -t "$agent" "clear" Enter
        $TMUX_BIN send-keys -t "$agent" "echo '═══════════════════════════════════════'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo '🤖 $agent Agent Terminal'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo '═══════════════════════════════════════'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo 'Status: ✅ Initialized'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo 'Project: RIONA AI'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo 'Ready for commands...'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo '═══════════════════════════════════════'" Enter
        $TMUX_BIN send-keys -t "$agent" "echo ''" Enter
...
full_setup() {
    ...
    if command -v jq &> /dev/null && [ -f "$INITIALIZER_SCRIPT" ]; then
        ...
        sed -i.bak "s|tmux |$TMUX_BIN |g" "$INITIALIZER_SCRIPT" 2>/dev/null || true
...
case "${1:-}" in
    ...
    "attach")
        ...
        $TMUX_BIN attach-session -t "$agent"
        ;;
    "menu")
        show_menu
